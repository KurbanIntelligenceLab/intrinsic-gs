...
def resolve_deform_weights_path(model_path, iteration=-1, name=None, scene_name=None):
    if scene_name:
        weights_path = os.path.join(model_path, "deform", f"deform_{scene_name}.pth")
    else:
        if iteration == -1:
    ...
    if not os.path.exists(weights_path):
        raise FileNotFoundError(f"Deform weights not found: {weights_path}")
    return weights_path
...
class DeformModel:
    ...
    def load_weights(self, model_path, iteration=-1, name=None, scene_name=None):
        weights_path = resolve_deform_weights_path(
            model_path, iteration=iteration, name=name, scene_name=scene_name
        )
        print(f"Loading deform weights: {weights_path}")
