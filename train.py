    ...
    if iteration in testing_iterations:
        ...
        validation_configs = ({'name': 'test',
                               'cameras': [scene.getTestCameras()[idx % len(scene.getTestCameras())] for idx in
                                           range(5, 30, 5)] if len(scene.getTestCameras()) > 0 else []},
                              {'name': 'train',
                               'cameras': [scene.getTrainCameras()[idx % len(scene.getTrainCameras())] for idx in
                                           range(5, 30, 5)] if len(scene.getTrainCameras()) > 0 else []})
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if not load_image_on_the_fly:
                        ...
                        if norm_data[:, :, 3:4].min() < 1:
                            arr = np.concatenate([arr, norm_data[:, :, 3:4]], axis=2)
                            gt_image = Image.fromarray(np.array(arr*255.0, dtype=np.uint8), "RGBA")
                        else:
                            gt_image = Image.fromarray(np.array(arr*255.0, dtype=np.uint8), "RGB")
